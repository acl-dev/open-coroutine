...
impl<'l, T: Debug> LocalQueue<'l, T> {
    ...
    ///
    /// ```
    /// use work_steal_queue_v2::WorkStealQueue;
    ///
    ...
    /// }
    /// assert_eq!(local0.pop_front(), Some(3));
    ...
    pub fn pop_front(&self) -> Option<T> {
        ...
        if self.try_lock() {
            ...
            for i in 0..num {
                ...
                if let Ok((popped_item, _)) = another.stealer().steal_and_pop(self.queue, |_| {
                    //本地队列空闲长度
                    self.queue
                        .spare_capacity()
                        //其他队列当前长度的一半
                        .min((another.capacity() - another.spare_capacity()) / 2)
                }) {
                    self.release_lock();
