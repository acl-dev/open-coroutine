...
///
/// ```
/// use work_steal_queue::{Steal, Worker};
///
...
impl<T> Worker<T> {
    ...
    ///
    /// ```
    /// use work_steal_queue::Worker;
    ///
    /// let w = Worker::<i32>::new_fifo();
    /// ```
    pub fn new_fifo() -> Worker<T> {
        Worker::new_capacity_fifo(MIN_CAP, true)
    }

    /// Creates a FIFO worker queue a init capacity.
    ///
    /// Tasks are pushed and popped from opposite ends.
    ///
    /// # Examples
    ...
    /// use work_steal_queue::Worker;
    ///
    /// let w = Worker::<i32>::new_capacity_fifo(1, false);
    /// assert_eq!(w.push(1), Ok(()));
    /// assert_eq!(w.push(2), Err(2));
    /// ```
    pub fn new_capacity_fifo(cap: usize, resizable: bool) -> Worker<T> {
    ...
    pub fn new_lifo() -> Worker<T> {
        Worker::new_capacity_lifo(MIN_CAP, true)
    }

    /// Creates a LIFO worker queue with a init capacity.
    ///
    /// Tasks are pushed and popped from the same end.
    ///
    /// # Examples
    ...
    /// use work_steal_queue::Worker;
    ///
    /// let w = Worker::<i32>::new_capacity_lifo(1, false);
    /// assert_eq!(w.push(1), Ok(()));
    /// assert_eq!(w.push(2), Err(2));
    /// ```
    pub fn new_capacity_lifo(cap: usize, resizable: bool) -> Worker<T> {
        let buffer = Buffer::alloc(cap);
    ...
    /// Reserves enough capacity so that `reserve_cap` tasks
    /// can be pushed without growing the buffer.
    ///
    /// Returns the remaining capacity.
    fn reserve(&self, reserve_cap: usize) -> usize {
        if reserve_cap > 0 {
...
impl<T> Stealer<T> {
    ...
    pub fn steal_batch(&self, dest: &Worker<T>) -> Steal<()> {
        ...
        let batch_size = ((len as usize + 1) / 2).min(MAX_BATCH);
        let remain = dest.reserve(batch_size);
        let mut batch_size = remain as isize;
    ...
    pub fn steal_batch_and_pop(&self, dest: &Worker<T>) -> Steal<T> {
        ...
        // Reserve capacity for the stolen batch.
        let batch_size = ((len as usize - 1) / 2).min(MAX_BATCH - 1);
        let remain = dest.reserve(batch_size);
        let mut batch_size = remain as isize;
...
///
/// ```
/// use work_steal_queue::{Injector, Steal};
///
/// let q = Injector::new();
/// q.push(1);
/// q.push(2);
...
impl<T> Injector<T> {
    ...
    ///
    /// ```
    /// use work_steal_queue::{Injector, Worker};
    ///
    /// let q = Injector::new();
    /// q.push(1);
    /// q.push(2);
    ...
    pub fn steal_batch(&self, dest: &Worker<T>) -> Steal<()> {
        ...
        // Reserve capacity for the stolen batch.
        let mut batch_size = new_offset - offset;
        batch_size = dest.reserve(batch_size);
    ...
    ///
    /// ```
    /// use work_steal_queue::{Injector, Steal, Worker};
    ///
    /// let q = Injector::new();
    /// q.push(1);
    /// q.push(2);
    ...
    pub fn steal_batch_and_pop(&self, dest: &Worker<T>) -> Steal<T> {
        ...
        // Reserve capacity for the stolen batch.
        let mut batch_size = new_offset - offset - 1;
        batch_size = dest.reserve(batch_size);
...
///
/// ```
/// use work_steal_queue::Steal::{self, Empty, Retry, Success};
///
...
impl<T> Steal<T> {
    ...
    ///
    /// ```
    /// use work_steal_queue::Steal::{Empty, Retry, Success};
    ///
